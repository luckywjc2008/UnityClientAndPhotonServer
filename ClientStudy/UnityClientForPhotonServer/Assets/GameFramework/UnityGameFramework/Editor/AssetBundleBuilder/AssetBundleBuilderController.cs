...
namespace UnityGameFramework.Editor.AssetBundleTools
{
    internal sealed partial class AssetBundleBuilderController
    {
        ...
        private void BuildAssetBundles(AssetBundleBuild[] buildMap, BuildAssetBundleOptions buildOptions, bool zip, BuildTarget buildTarget)
        {
            ...
            string buildTargetUrlName = UnityUtility.EditorPath.GetBuildTargetName(buildTarget);
            //!WJC
        ...
        private AssetBundleBuild[] GetBuildMap()
        {
            ...
            foreach (Asset asset in assets)
            {
                ...
                {
                    //WJC
                    if (assetName != dependencyAsset.Name)
                    {
                        dependencyAssetNames.Add(dependencyAsset.Name);
                    }
                    else
                    {
                        m_BuildReport.LogWarning("{0} Asset dependency myself", dependencyAsset.Name);
                    }
                    //dependencyAssetNames.Add(dependencyAsset.Name);
                    //!WJC
                }

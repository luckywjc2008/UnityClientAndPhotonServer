...
namespace UnityGameFramework.Runtime
{
    ...
    public sealed partial class ResourceComponent : GameFrameworkComponent
    {
        ...
        /// 获取或设置资源加载代理的个数
        /// </summary>
        public int LoadResourceAgentHelperCount
        {
            get
            {
                return m_LoadResourceAgentHelperCount;
            }
            set
            {
                m_LoadResourceAgentHelperCount = value;
            }
        }
        //!NOWGF
        ...
        private void Start()
        {
            ...
            //WJC
            string combinedTempPath = Utility.Path.GetCombinePath(Application.streamingAssetsPath, Constant.DevicePlatform.GetCurDevicePlatformPath());
            m_ResourceManager.SetReadOnlyPath(combinedTempPath);
            if (m_ReadWritePathType == ReadWritePathType.TemporaryCache)
            {
                combinedTempPath = Utility.Path.GetCombinePath(Application.temporaryCachePath, Constant.DevicePlatform.GetCurDevicePlatformPath());
                m_ResourceManager.SetReadWritePath(combinedTempPath);
            }
            else
            {
                ...
                combinedTempPath = Utility.Path.GetCombinePath(Application.persistentDataPath, Constant.DevicePlatform.GetCurDevicePlatformPath());
                m_ResourceManager.SetReadWritePath(combinedTempPath);
            }

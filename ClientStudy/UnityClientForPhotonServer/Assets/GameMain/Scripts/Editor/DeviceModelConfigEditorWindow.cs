...
namespace StarForce.Editor
{
    ...
    {
        private DeviceModelConfigCustom _mConfigCustom = null;

        [MenuItem("Star Force/Device Model ConfigCustom Editor")]
        public static void EditDeviceModelConfig()
        {
            const string DeviceModelConfigFullName = "Assets/GameMain/Configs/DeviceModelConfigObject.asset";
            OpenWindow(AssetDatabase.LoadAssetAtPath<DeviceModelConfigCustom>(DeviceModelConfigFullName));
        }

        public static void OpenWindow(DeviceModelConfigCustom deviceModelConfigCustom)
        {
            if (deviceModelConfigCustom == null)
            {
                return;
            }

            DeviceModelConfigEditorWindow window = GetWindow<DeviceModelConfigEditorWindow>(true, "Device Model ConfigCustom Editor");
            window._mConfigCustom = deviceModelConfigCustom;
            window.minSize = new Vector2(460f, 400f);
        }

        private void OnGUI()
        {
            if (_mConfigCustom == null)
            {
                return;
            }
        ...
        {
            DeviceModel[] deviceModels = _mConfigCustom.GetDeviceModels();
            ...
            if (deleteIndex >= 0)
            {
                _mConfigCustom.RemoveDeviceModelAt(deleteIndex);
            }

            if (GUILayout.Button("+", GUILayout.Width(20f)))
            {
                _mConfigCustom.NewDeviceModel();
            }
        ...
        private void DrawTextItem(object obj, FieldInfo field, float width = 300)
        {
            ...
            if (value != oldValue)
            {
                EditorUtility.SetDirty(_mConfigCustom);
            }
